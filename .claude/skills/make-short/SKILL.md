...
captions and chips clear of `lib/shorts.tsx`'s `SAFE` margins (top ~180px, bottom ~500px,
right ~170px — the username/caption/music row and the like/comment/share column on TikTok/
Reels/Shorts; a caption placed by a bespoke y-override instead of the `Captions` default is
the most common way this regresses — see vox-2's script.md for a real case) · loop frame ≈
frame 0. Fix, re-render frames, only then:
...
- For more expressive delivery, add `--model eleven_v3` and give lines in beats.json a `tts`
  field (e.g. `"tts": "[curious] ..."`) carrying inline delivery tags around the clean `text` —
  tags are stripped from the caption word map automatically. Premade voices work with v3 on the
  direct API key; a Voice Library voice may 402 on the free tier — use the ElevenLabs MCP
  connector instead (see short-14's script.md for the cache-injection workaround).
...
- **Continuous narration, two-pass:** a `start` picked from a words/sec estimate is a ceiling,
  not a prediction — real speech usually finishes faster, and since the next line's start is
  fixed, that slack becomes audible dead air (verify with `ffmpeg -af silencedetect`; an
  estimate-only timeline measured at 1.2-2.1s gaps between lines on one video, a corrected one
  held every gap to 0.16-0.3s). Fix: generate once to read real per-line durations back from
  beats.json, rewrite every `start` to the previous line's real end + ~0.12s, regenerate
  (cached mp3s key off voice+model+text, not start time, so this doesn't re-bill ElevenLabs) —
  then retime every frame-based cue that referenced the old loose timeline.
