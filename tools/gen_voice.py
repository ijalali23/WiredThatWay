...
def main():
    ...
    for i, line in enumerate(vo):
        ...
        model = line.get("model", args.model)  # optional per-line model override
        tts_text = line.get("tts", line["text"])  # optional tagged/phonetic variant for TTS
        h = hashlib.sha1(f"{args.voice}|{model}|{tts_text}".encode()).hexdigest()[:8]
        ...
        if args.force or not os.path.exists(raw) or not os.path.exists(raw + ".words.json"):
            ...
            tts_line(key, args.voice, model, tts_text, prev_text, next_text, raw)
