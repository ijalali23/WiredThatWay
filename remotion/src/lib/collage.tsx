...
// -----------------------------------------------------------------------------
// StatCard — a plain typographic reveal card (a headline stat + optional sub-line), no
// photo/emblem/people. Promoted here from vox-3 once vox-4 needed the exact same thing:
// per make-vox's own rule, a project-specific hack that a second video reuses graduates to
// the shared kit. Board-positioned like every other collage element (x/y = center).
// -----------------------------------------------------------------------------
export const StatCard: React.FC<{
  ...
  sub?: string;
  w?: number;
}> = ({ x, y, at, kicker, big, sub, w = 760 }) => {
  const frame = useCurrentFrame();
  const p = EASE_OUT(Math.max(0, Math.min(1, (frame - at) / 16)));
  if (frame < at) return null;
  return (
    <div
      style={{
    ...
    >
      <div style={{ fontFamily: FONT_BODY, fontWeight: 700, fontSize: 26, letterSpacing: 3, color: VOX.inkSoft, textTransform: 'uppercase' }}>
        {kicker}
      </div>
      <div style={{ fontFamily: FONT_EDITORIAL, fontWeight: 900, fontSize: 108, color: VOX.ink, lineHeight: 1.05, marginTop: 14 }}>
        {big}
      </div>
      {sub ? (
        <div style={{ fontFamily: FONT_BODY, fontWeight: 600, fontSize: 30, color: VOX.inkSoft, marginTop: 18 }}>{sub}</div>
      ) : null}
    </div>
  );
};
