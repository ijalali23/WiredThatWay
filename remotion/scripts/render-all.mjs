...
// Optional overrides for environments where Remotion can't auto-download its
// headless Chrome (no network access to remotion.media) or where a TLS-intercepting
// proxy breaks cert validation for requests the browser makes (e.g. Google Fonts).
// Unset in normal environments, where Remotion downloads and trusts its own browser.
const browserExecutable = process.env.REMOTION_BROWSER_EXECUTABLE || undefined;
const chromiumOptions = process.env.REMOTION_IGNORE_CERTIFICATE_ERRORS === '1'
...
for (const shot of manifest) {
  ...
  const composition = await selectComposition({ serveUrl, id: shot.id, browserExecutable, chromiumOptions });
